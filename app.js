(() => {
  ...
  function birthday(){
    root.innerHTML=`<section class="birthday"><img class="birthdayMark" src="./assets/octolearn-mark.svg" alt="OctoLearn"/><h1 class="homeTitle">Төрсөн өдрөө оруулна уу</h1><p class="sub">Бид таны суралцах туршлагыг хувийн болгоход ашиглана</p><div class="wheel"><div><span>1988</span><span>1989</span><strong>1990</strong><span>1991</span><span>1992</span></div><div><span>3 сар</span><span>4 сар</span><strong>5 сар</strong><span>6 сар</span><span>7 сар</span></div><div><span>1</span><span>2</span><strong>3</strong><span>4</span><span>5</span></div></div><div class="selectedDate">1990 оны 05 сарын 03</div><button class="primary orange" id="continue">Үргэлжлүүлэх</button></section>`;
    document.getElementById('continue').onclick=()=>{state.screen='lesson';save();renderLesson();};
  }
  function replay(){ if(!('speechSynthesis' in window)) return; window.speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(lessons[state.lessonIndex].text);u.lang='cs-CZ';u.rate=.82;window.speechSynthesis.speak(u); }
  function checkCompletion(){ const targets=targetWords(); if(state.correctWordIndices.length!==targets.length || state.isLessonCompleted) return; state.isLessonCompleted=true; renderLesson(); window.setTimeout(()=>{state.lessonIndex=(state.lessonIndex+1)%lessons.length;state.correctWordIndices=[];state.isLessonCompleted=false;state.listening=false;state.error='';renderLesson();},1500); }
  function startRecognition(){
    if(state.isLessonCompleted) return; const Recognition=speech(); if(!Recognition){state.error='Энэ browser microphone speech recognition дэмжихгүй байна.';renderLesson();return;} state.error='';state.listening=true;state.correctWordIndices=[];renderLesson();
    ...
    recognition.onerror=(event)=>{state.listening=false;state.error=event.error==='not-allowed'?'Микрофоны зөвшөөрөл өгөөгүй байна.':'Дуу танигдсангүй. Дахин удаан хэлж үзээрэй.';renderLesson();}; recognition.onend=()=>{if(!state.isLessonCompleted){state.listening=false;renderLesson();}};recognition.start();
  }
  function renderLesson(){
    const lesson=lessons[state.lessonIndex]; const buttonClass=state.isLessonCompleted?'primary success':state.listening?'primary listening':'primary'; const buttonText=state.isLessonCompleted?'✓ Үргэлжлүүлэх':state.listening?'🎙 Сонсож байна...':'🎤 Дуудлага шалгах'; const display=lesson.text.replace(/^([^ ]+)/,'<span class="hit">$1</span>');
    root.innerHTML=`${header(state.lessonIndex+1)}<section class="octoTip"><img class="octoLogo" src="./assets/octolearn-mark.svg" alt="OctoLearn багш"/><div class="tip">${lesson.tip}</div></section><section class="phrase"><h1>${display}</h1><p class="translation">${lesson.mn}</p><button class="replay" id="replay">🔊</button><div class="replayLabel">Дахин сонсох</div></section><div class="actions"><div class="secondary"><button id="flash">🗂 Флэшкарт+</button><button id="skip">⏭ Алгасах</button></div>${state.error?`<p class="notice">${state.error}</p>`:''}<button class="${buttonClass}" id="speak">${buttonText}</button></div>`;
    document.getElementById('replay').onclick=replay;document.getElementById('speak').onclick=startRecognition;document.getElementById('skip').onclick=()=>{state.lessonIndex=(state.lessonIndex+1)%lessons.length;state.correctWordIndices=[];renderLesson();};document.getElementById('flash').onclick=()=>alert(`${lesson.text}\n${lesson.mn}`);document.getElementById('close').onclick=()=>{state.screen='levels';save();levels();};
  }
  function boot(){if(state.screen==='birthday')birthday();else if(state.screen==='lesson')renderLesson();else levels();} boot();
})();